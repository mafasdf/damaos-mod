/*
 * TradingMarket.java Created on February 18, 2007, 2:29 PM
 */
...
public class TradingMarket
{
	...
	public TradingMarket(SimSpecs simSpec)
	{
		...
		for(int m = 0; m < buyerReserve.length && m < sellerReserve.length && buyerReserve[buyerReserve.length - 1 - m] >= sellerReserve[m]; m++)
		{
		...
		for(int i = 0; i < buyerReserve.length && i < sellerReserve.length && cmcPrice == -1; i++)
		{
					...
					&& -1 == cmcPrice) // if no cmcPoints
			{
				...
				else
				{
					...
					// seller i + 1's price is loser than buyer i+1's price
					{
	...
	public void trade()
	{
		...
		if(conciseOutputMode)
		{
			...
			//quick & dirty way of outputting individual round totals separately
			//        	if(marketID == 0)
			//        		System.err.print("\n" + dayCounter.getValue() + "\t" +  sellerTotalProfits[0]);
			//        	else System.err.print("\t" + sellerTotalProfits[0]);
		}
		else
		{
			// Output to comma separated file
			// System.out is redirected to the output file in the TradingWorld class
			System.out.println(dayCounter.getValue() + "," + stats.GetNumTrans() + "," + TWO_DIGITS.format(stats.GetAverage()) + ","
					+ TWO_DIGITS.format(marketsurplus) + "," + THREE_DIGITS.format(stats.GetMarketEfficiency()) + ","
					+ TWO_DIGITS.format(stats.GetStdDev()) + "," + TWO_DIGITS.format(stats.getBuyerActualSurplus()) + ","
					+ TWO_DIGITS.format(stats.getSellerActualSurplus()) + "," + "BuyerAdvantages:"
					+ formatNumberArray(calculateAdvantages(stats.getIndividualBuyersMarketSurplus(), buyerCMCSurplus, maxMarketSurplus)) + ","
					+ "SellerAdvantages:"
					+ formatNumberArray(calculateAdvantages(stats.getIndividualSellersMarketSurplus(), sellerCMCSurplus, maxMarketSurplus)));
		}
	...
	{
		if(toAddTo.length != terms.length) throw new IllegalArgumentException("Array size mismatch");
	...
	{
		if(individualMarketSurpluses.length != idealSurpluses.length) throw new IllegalArgumentException("Mismatching array sizes");
	...
	private BuyerAgent parseAgentSpecsToBuyer(AgentSpecs ac, MarketSpecs mc)
	{
		...
		{
			tradingStyle = new ChangingZI(value, ac.actionDomainParameters, ac.actionDomainSecondaryParameters, tsSeed, AgentType.BUYER, dayCounter,
					ac.changeDay);
		}
		...
		{
			tradingStyle = new TwoLayerModRothErev(value, ac.actionDomainParameters, tsSeed, (MultiLayerMREParams) ac.learningStyleParameters,
					AgentType.BUYER, ac.recencyDomainParameters, ac.experimentationDomainParameters, ac.daysBetweenChanges, dayCounter);
		}
	...
	private SellerAgent parseAgentSpecsToSeller(AgentSpecs ac, MarketSpecs mc)
	{
		...
		{
			tradingStyle = new ChangingZI(value, ac.actionDomainParameters, ac.actionDomainSecondaryParameters, tsSeed, AgentType.SELLER, dayCounter,
					ac.changeDay);
		}
		else if(tradeStyleName.equalsIgnoreCase("ModifiedRothErev"))
		{
		...
		{
			tradingStyle = new TwoLayerModRothErev(value, ac.actionDomainParameters, tsSeed, (MultiLayerMREParams) ac.learningStyleParameters,
					AgentType.SELLER, ac.recencyDomainParameters, ac.experimentationDomainParameters, ac.daysBetweenChanges, dayCounter);
		}
		else if(tradeStyleName.equalsIgnoreCase("SimpleBeliefBased"))
		{
